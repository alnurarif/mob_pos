    ...
    public function __construct()
    {
        ...
        if (!$this->logged_in() && get_cookie('identity') && get_cookie('remember_code')) {
            if ($this->auth_model->login_remembered_user()) {
                redirect($this->session->userdata('requested_page') ? $this->session->userdata('requested_page') : (isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : 'welcome'));
            }
        }
        ...
        if ($this->auth_model->forgotten_password($identity)) {   //changed
            ...
            if ($user) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
                    ...
                    $msg     = file_get_contents('./themes/' . $Settings->theme . '/views/email_templates/forgot_password.html');
    ...
    public function forgotten_password_complete($code)
    {
        ...
        if ($new_password) {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                $msg     = file_get_contents('./themes/' . $Settings->theme . '/views/email_templates/new_password.html');
                $message = $this->parser->parse_string($msg, $parse_data);
                $subject = lang('email_new_password_subject') . ' - ' . $Settings->title;

                try {
                    if ($this->repairer->send_email($profile->email, $subject, $message)) {
                        $this->set_message('password_change_successful');
                        $this->auth_model->trigger_events(['post_password_change', 'password_change_successful']);
                        return true;
                    } else {
        ...
        if (!$email_activation || $active == '1') {
            ...
            if ($id !== false) {
                if ($notify) {
                    ...
                    $msg     = file_get_contents('./themes/' . $Settings->theme . '/views/email_templates/credentials.html');
                    $message = $this->parser->parse_string($msg, $parse_data);
                    $subject = $this->lang->line('new_user_created') . ' - ' . $Settings->title;
                    try {
                        $this->repairer->send_email($email, $subject, $message);
                    } catch (Exception $e) {
                        $this->set_error($e->getMessage());
                    }
                }
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                $msg     = file_get_contents('./themes/' . $Settings->theme . '/views/email_templates/activate_email.html');
                $message = $this->parser->parse_string($msg, $parse_data);
                $subject = $this->lang->line('email_activation_subject') . ' - ' . $Settings->title;

                try {
                    if ($this->repairer->send_email($email, $subject, $message)) {
                        $this->auth_model->trigger_events(['post_account_creation', 'post_account_creation_successful', 'activation_email_successful']);
                        $this->set_message('activation_email_successful');
                        return $id;
                    }
