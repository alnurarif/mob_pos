...
$(document).ready(function () {
    <?php if ($inv) { ?>
        localStorage.setItem('podate', '<?php echo date($dateFormats['php_ldate'], strtotime($inv->date))?>');
        localStorage.setItem('posupplier', '<?php echo $inv->supplier_id?>');
        localStorage.setItem('poref', '<?php echo $inv->reference_no?>');
        localStorage.setItem('postatus', '<?php echo $inv->status?>');
        localStorage.setItem('ponote', '<?php echo str_replace(array("\r", "\n"), "", $this->repairer->decode_html($inv->note)); ?>');
        localStorage.setItem('podiscount', '<?php echo $inv->order_discount_id?>');
        localStorage.setItem('potax2', '<?php echo $inv->order_tax_id?>');
        localStorage.setItem('poshipping', '<?php echo $inv->shipping?>');
        ...
        localStorage.setItem('poitems', JSON.stringify(<?php echo $inv_items;?>));
    <?php } ?>
});
</script>
<?php echo $this->load->view($theme.'purchases/js'); ?>
<script type="text/javascript">
    var count = 1, an = 1, po_edit = true, product_variant = 0, DT = <?php echo $settings->default_tax_rate ?>, DC = '<?php echo escapeStr($settings->currency)?>', shipping = 0,
    ...
    $(document).ready(function () {
        ...
        $("#add_item").autocomplete({
            source: '<?php echo site_url('panel/purchases/suggestions'); ?>',
            minLength: 1,
            autoFocus: false,
            delay: 250,
        ...
        $(window).on('beforeunload', function (e) {
            $.get('<?php echo site_url('welcome/set_data/remove_pols/1');?>');
            if (count > 1) {
        ...
        $('#edit_pruchase').on( "click", function () {
            $(window).unbind('beforeunload');
    ...
    <div class="box-body">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <?php
                $attrib = array('data-toggle' => 'validator', 'role' => 'form', 'class' => 'edit-po-form', 'id'=>'purchase_form');
                        ...
                        <div class="col-md-4">
                         ...
                         <div class="col-md-12">
                            <div class="panel panel-warning">
                                <div class="panel-heading"><?php echo lang('please_select_these_before_adding_product'); ?></div>
                                ...
                                <div class="controls table-controls">
                                    <table id="poTable"
                                        ...
                                        <tr>
                                            ...
                                            <th><div class="pull-right"><?php echo lang("subtotal"); ?> (<span class="currency"><?php echo escapeStr($settings->currency) ?></span>)</div></th>
                            ...
                            <div class="row" id="extras-con" style="display: none;">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <?php echo form_input('order_tax', set_value('order_tax', $inv->order_tax), 'class="form-control input-tip" id="potax2"'); ?>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <?php echo form_input('track_code', set_value('track_code', $inv->track_code), 'class="form-control"'); ?>
                                    </div>
                                </div>
