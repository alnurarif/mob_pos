            ...
            <div class="content">
                <table>
                    <tr>
                        <td>
                            <h2>
                                <center><img src="<?php echo base_url().'assets/uploads/logos/' . $settings->logo; ?>" alt="<?php echo escapeStr($settings->title); ?>"></center>
                            </h2>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <div style="clear:both;height:15px;"></div>
                                <div id="receiptData" style="border:1px solid #DDD; padding:10px; text-align:center;">
                                    ...
                                    <div class="text-center">
                                        <h3 style="text-transform:uppercase;"><?php echo escapeStr($settings->title); ?></h3>
                                        <address>
                                    ...
                                    <table width="100%" style="margin:15px 0;">
                                        ...
                                        foreach ($rows as $row) {
                                            ...
                                            if ($row->item_discount != 0) {
                                             ...
                                             if ($inv->surcharge != 0) {
                                                echo '<tr><th style="text-align:left;border-bottom:1px solid #DDD;">' . lang('Surcharge') . '</th><th style="text-align:right;border-bottom:1px solid #DDD;">' . $this->repairer->formatMoney($inv->surcharge) . '</th></tr>';
                                            }
                            ...
                            if ($payments) {
                                ...
                                echo '<thead><tr><td colspan="3">Payments</td></tr></thead>';
                                foreach ($payments as $payment) {
                                    ...
                                    if (($payment->paid_by == 'cash' || $payment->paid_by == 'ppp' || $payment->paid_by == 'CC') && $payment->pos_paid) {
                                        ...
                                        echo '<td>' . "Change" . ': ' . $this->repairer->formatMoney($payment->pos_balance > 0 ? ($payment->pos_balance) : 0) . '</td>';
                                        ...
                                        echo '<td class="">' . lang('paid_by') . ': ' . lang($payment->paid_by) . '</td>';
                                        echo '<td>' . lang('Amount') . ': ' . $this->repairer->formatMoney($payment->pos_paid == 0 ? $payment->amount : $payment->pos_paid)  . '</td>';
                                        echo $payment->note ? '</tr><td colspan="2">' . lang('Payment Note') . ': ' . escapeStr($payment->note) . '</td>' : '';
                                    }
                                    echo '</tr>';
                                }
                                echo '</tbody></table>';
                            ...
                            <div style="clear:both;height:15px;"></div>
                            <p style="border-top:1px solid #CCC;margin-bottom:0;">This email is sent to <?php echo escapeStr($customer->first_name).' '.escapeStr($customer->last_name); ?> (<?php echo $customer->email; ?>).</p>
                        </td>
                    </tr>
                </table>
            </div>
